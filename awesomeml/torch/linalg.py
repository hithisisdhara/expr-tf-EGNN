# -*- coding: utf-8 -*-
"""
Linear algebra operations on PyTorch tensors.
"""
import itertools
import torch
...
def gram_schmidt(X, axis1=0, axis2=None, normalize=True):
    ...
    Args:
      ...
      X (tensor): Input matrix (or matrices).
    ...
    >>> import numpy as np
    >>> X = torch.rand(3)
    >>> Y = gram_schmidt(X).numpy()
    >>> np.testing.assert_almost_equal(np.sum(np.square(Y)), 1, 3)
    >>> X = torch.rand(3, 3)
    >>> Y = gram_schmidt(X).numpy()
    >>> np.testing.assert_almost_equal(Y.T @ Y, np.eye(3), 3)
    >>> X = torch.rand(3, 3, 6)
    >>> Y = gram_schmidt(X, axis1=(0, 1)).numpy()
    >>> Y = np.reshape(Y, [9, 6])
    >>> np.testing.assert_almost_equal(Y.T @ Y, np.eye(6), 3)
    >>> X = torch.rand(3, 3, 3, 3)
    >>> Y = gram_schmidt(X, axis1=None, axis2=(2, 3)).numpy()
    >>> Y = np.reshape(Y, [9, 9])
    >>> np.testing.assert_almost_equal(Y.T @ Y, np.eye(9), 3)
    >>> X = torch.rand(3, 3, 3, 3)
    >>> Y = gram_schmidt(X, axis1=0, axis2=2).numpy()
    >>> for i in range(3):
    ...     for j in range(3):
    ...         Y2 = Y[:, i, :, j]
    ...         np.testing.assert_almost_equal(Y2.T @ Y2, np.eye(3), 3)
    ...
    def dot(u, v):
        return torch.sum(u * v, tuple(axis1), keepdim=True)
    ...
    # normalization
    if normalize:
        U /= torch.sqrt(torch.sum(U*U, tuple(axis1), keepdim=True))
    return U
